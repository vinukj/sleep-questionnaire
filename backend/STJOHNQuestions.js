// STJohnQuestionnaire.js
export const STJohnQuestionnaire = [
  {
    page: 1,
    title: "Patient Information",
    questions: [
      { id: "hospital_id", type: "text", label: "Hospital ID" },
      { id: "name", type: "text", label: "Name" },
      { id: "gender", type: "radio", label: "Gender", options: ["M", "F"] },
      { id: "age", type: "number", label: "Age" },
      { id: "occupation", type: "text", label: "Occupation" },
      { id: "phone", type: "tel", label: "Phone Number" },
      { id: "email", type: "email", label: "Email ID" },
    ],
  },
  {
    page: 2,
    title: "Sleep Habits",
    questions: [
      { id: "bedtime", type: "time", label: "Bedtime" },
      {
        id: "sleep_latency",
        type: "radio",
        label: "Sleep latency/Time taken to fall asleep",
        options: ["< 15 min", "15-30 min", "> 30 min"],
      },
      {
        id: "night_awakenings",
        type: "radio",
        label: "Number of awakenings at night",
        options: ["None", "1-2", ">2 times"],
      },
      { id: "avg_sleep_hours", type: "number", label: "Average hours of sleep" },
      {
        id: "shift_worker",
        type: "radio",
        label: "Are you a shift worker?",
        options: ["Yes", "No"],
      },
      {
        id: "shift_pattern",
        type: "radio",
        label: "If yes, shift pattern",
        options: ["Rotating (day/night)", "Fixed night shift"],
        dependsOn: { id: "shift_worker", value: "Yes" },
      },
    ],
  },
  {
    page: 3,
    title: "Main Sleep Complaints",
    questions: [
      { id: "difficulty_initiating", type: "radio", label: "Difficulty initiating (falling) sleep", options: ["Yes", "No"] },
      { id: "difficulty_maintaining", type: "radio", label: "Difficulty maintaining (staying) sleep", options: ["Yes", "No"] },
      { id: "daytime_sleepiness", type: "radio", label: "Daytime sleepiness", options: ["Yes", "No"] },
      { id: "snoring", type: "radio", label: "Loud or intrusive snoring for more than 3 nights a week.", options: ["Yes", "No"] },
      { id: "witnessed_apneas", type: "radio", label: "Witnessed apneas", options: ["Yes", "No"] },
      { id: "nocturnal_choking", type: "radio", label: "Nocturnal choking episodes", options: ["Yes", "No"] },
      {
        id: "restless_legs",
        type: "radio",
        label: "Restless legs symptoms (Uncomfortable sensation, worse in evening/night, relieved on moving)",
        options: ["Yes", "No"],
      },
      { id: "parasomnias", type: "radio", label: "Parasomnias", options: ["Yes", "No"] },
      {
        id: "parasomnia_details",
        type: "checkbox",
        label: "If yes, specify",
        options: ["Sleep walking", "Sleep talking", "Sleep eating", "Nightmares can appear"],
        dependsOn: { id: "parasomnias", value: "Yes" },
      },
      { id: "narcolepsy", type: "radio", label: "Narcolepsy symptoms", options: ["Yes", "No"] },
      {
        id: "narcolepsy_details",
        type: "checkbox",
        label: "If yes, specify",
        options: ["Sleep attacks", "Sleep paralysis", "Cataplexy", "Hallucinations"],
        dependsOn: { id: "narcolepsy", value: "Yes" },
      },
      { id: "rem_disorder", type: "radio", label: "REM Behaviour disorder symptoms (Acting out dreams)", options: ["Yes", "No"] },
      { id: "bruxism", type: "radio", label: "Bruxism (grinding teeth in sleep)", options: ["Yes", "No"] },
    ],
  },
  {
    page: 4,
    title: "Co-morbidities and Medications",
    questions: [
      { id: "hypertension", type: "radio", label: "Hypertension", options: ["Yes", "No"] },
      { id: "diabetes", type: "radio", label: "Diabetes", options: ["Yes", "No"] },
      { id: "ihd", type: "radio", label: "Ischemic Heart Disease", options: ["Yes", "No"] },
      { id: "stroke", type: "radio", label: "Stroke", options: ["Yes", "No"] },
      { id: "hypothyroidism", type: "radio", label: "Hypothyroidism", options: ["Yes", "No"] },
      {
        id: "neurological_disorder",
        type: "checkbox",
        label: "Neurological disorder",
        options: ["Parkinson’s", "Dementia", "Neuromuscular disease"],
      },
      {
        id: "respiratory_disorder",
        type: "checkbox",
        label: "Chronic Respiratory disorder",
        options: ["COPD", "Asthma", "Interstitial Lung Disease", "Bronchiectasis"],
      },
      { id: "surgery_sleep_apnea", type: "radio", label: "Surgery for Sleep Apnea", options: ["Yes", "No"] },
      {
        id: "medications",
        type: "checkbox",
        label: "Are you taking medications affecting sleep?",
        options: ["Sedative-hypnotics", "Antidepressants", "Antipsychotics", "Stimulants", "Opioids", "Others"],
      },
      {
        id: "medications_other",
        type: "text",
        label: "Other (please specify)",
        dependsOn: { id: "medications", value: "Others" },
      },
    ],
  },
  {
    page: 5,
    title: "Clinical Examination",
    questions: [
      { id: "height", type: "number", label: "Height (cm)" },
      { id: "weight", type: "number", label: "Weight (kg)" },
      {
        id: "bmi",
        type: "number",
        label: "BMI (kg/sq.m)",
        calculated: true,
        dependsOn: ["height", "weight"],
        formula: "weight / ((height/100) * (height/100))",
      },
      { id: "waist", type: "number", label: "Waist circumference (cm)" },
      { id: "hip", type: "number", label: "Hip circumference (cm)" },
      {
        id: "waist_hip_ratio",
        type: "number",
        label: "Waist/Hip ratio",
        calculated: true,
        dependsOn: ["waist", "hip"],
        formula: "waist / hip",
      },
      { id: "neck", type: "number", label: "Neck circumference (cm)" },
      { id: "bp", type: "text", label: "Blood Pressure" },
      { id: "spo2", type: "number", label: "SpO2" },
      { id: "mallampati", type: "radio", label: "Mallampati Grade", options: ["1", "2", "3", "4"] },
    ],
  },
  {
    page: 6,
    title: "Sleepiness Scales",
    questions: [
      { id: "epworth_score", type: "number", label: "Epworth Sleepiness Scale Score" },
      { id: "iss_q1", type: "radio", label: "On waking, do you feel drowsy/unrefreshing sleep?", options: ["Yes", "No"] },
      { id: "iss_q2a", type: "radio", label: "Do you snore?", options: ["Yes", "No"] },
      { id: "iss_q2b", type: "radio", label: "If yes, has your bed partner noticed that you stop breathing while snoring?", options: ["Yes", "No"], dependsOn: { id: "iss_q2a", value: "Yes" } },
      { id: "iss_q3", type: "radio", label: "Do your knees give way during emotional situations?", options: ["Yes", "No"] },
      { id: "iss_q4", type: "radio", label: "Do you kick your bed partner/bed sheets away in sleep?", options: ["Yes", "No"] },
      { id: "iss_q5", type: "radio", label: "Do you feel drowsy while watching TV or leisure activities?", options: ["Yes", "No"] },
      { id: "iss_q6", type: "radio", label: "Do you feel drowsy / sleepy while doing household work?", options: ["Yes", "No"] },
      { id: "iss_q7", type: "radio", label: "Have you ever experienced sleep attacks?", options: ["Yes", "No"] },
      { id: "iss_q8a", type: "radio", label: "Nodded off / Felt sleepy while conversing?", options: ["Yes", "No"] },
      { id: "iss_q8b", type: "radio", label: "Nodded off / Felt sleepy while driving?", options: ["Yes", "No"] },
      { id: "iss_q8c", type: "radio", label: "Nodded off / Felt sleepy while cooking?", options: ["Yes", "No"] },
      { id: "iss_q8d", type: "radio", label: "Nodded off / Felt sleepy while office work?", options: ["Yes", "No"] },
      { id: "iss_q8e", type: "radio", label: "Nodded off / Felt sleepy while operating heavy machinery?", options: ["Yes", "No"] },
    ],
  },
  {
    page: 7,
    title: "Clinical Impression & Work-up",
    questions: [
      {
        id: "clinical_impression",
        type: "checkbox",
        label: "Clinical Impression",
        options: [
          "Poor Sleep Hygiene",
          "Obstructive Sleep Apnea",
          "Obesity Hypoventilation Syndrome",
          "Insomnia",
          "Restless Legs Syndrome",
          "Parasomnia",
          "RBD",
          "Narcolepsy",
          "Circadian Rhythm Sleep Disorder",
          "Shift Work Sleep Disorder",
          "Others",
        ],
      },
      { id: "clinical_impression_other", type: "text", label: "Other (please specify)", dependsOn: { id: "clinical_impression", value: "Others" } },
      {
        id: "recommended_workup",
        type: "checkbox",
        label: "Recommended work-up",
        options: [
          "Level 1 Polysomnography",
          "Level 2 Polysomnography",
          "Level 3 Polysomnography",
          "Split night polysomnography",
          "Multiple Sleep Latency Test",
          "PAP titration study",
        ],
      },
    ],
  },
];


// export const STJohnQuestionnaire = [
//   {
//     page: 1,
//     title: "Patient Information",
//     questions: [
//       {
//         id: "hospital_id",
//         type: "text",
//         label: "Hospital ID",
//         required: false,
//       },
//       { id: "name", type: "text", label: "Name", required: false },
//       {
//         id: "gender",
//         type: "radio",
//         label: "Gender",
//         options: ["M", "F"],
//         required: false,
//       },
//       { id: "age", type: "number", label: "Age", required: false },
//       { id: "occupation", type: "text", label: "Occupation", required: false },
//       { id: "phone", type: "tel", label: "Phone Number", required: false },
//       { id: "email", type: "email", label: "Email ID", required: false },
//     ],
//   },
//   {
//     page: 2,
//     title: "Sleep Habits",
//     questions: [
//       { id: "bedtime", type: "time", label: "Bedtime", required: false },
//       {
//         id: "sleep_latency",
//         type: "radio",
//         label: "Sleep latency/Time taken to fall asleep",
//         options: ["< 15 min", "15-30 min", "> 30 min"],
//         required: false,
//       },
//       {
//         id: "night_awakenings",
//         type: "radio",
//         label: "Number of awakenings at night",
//         options: ["None", "1-2", ">2 times"],
//         required: false,
//       },
//       {
//         id: "avg_sleep_hours",
//         type: "number",
//         label: "Average hours of sleep",
//         required: false,
//       },
//       {
//         id: "shift_worker",
//         type: "radio",
//         label: "Are you a shift worker?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "shift_pattern",
//         type: "radio",
//         label: "If yes, shift pattern",
//         options: ["Rotating (day/night)", "Fixed night shift"],
//         dependsOn: { id: "shift_worker", value: "Yes" },
//         required: false,
//       },
//     ],
//   },
//   {
//     page: 3,
//     title: "Main Sleep Complaints",
//     questions: [
//       {
//         id: "difficulty_initiating",
//         type: "radio",
//         label: "Difficulty initiating (falling) sleep",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "difficulty_maintaining",
//         type: "radio",
//         label: "Difficulty maintaining (staying) sleep",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "daytime_sleepiness",
//         type: "radio",
//         label: "Daytime sleepiness",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "snoring",
//         type: "radio",
//         label: "Loud or intrusive snoring for more than 3 nights a week.",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "witnessed_apneas",
//         type: "radio",
//         label: "Witnessed apneas",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "nocturnal_choking",
//         type: "radio",
//         label: "Nocturnal choking episodes",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "restless_legs",
//         type: "radio",
//         label:
//           "Restless legs symptoms (Uncomfortable or unpleasant sensation in the legs, more in the evening/night, symptoms relieved on moving around)",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "parasomnias",
//         type: "radio",
//         label: "Parasomnias",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "parasomnia_details",
//         type: "checkbox",
//         options: [
//           "Sleep walking",
//           "Sleep talking",
//           "Sleep eating",
//           "Nightmares can appear",
//         ],
//         label: "If yes, specify",
//         dependsOn: { id: "parasomnias", value: "Yes" },
//         required: false,
//       },
//       {
//         id: "narcolepsy",
//         type: "radio",
//         label: "Narcolepsy symptoms",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "narcolepsy_details",
//         type: "checkbox",
//         options: [
//           "Sleep attacks",
//           "Sleep paralysis",
//           "Cataplexy",
//           "Hallucinations",
//         ],
//         label: "If yes, specify",
//         dependsOn: { id: "narcolepsy", value: "Yes" },
//         required: false,
//       },
//       {
//         id: "rem_disorder",
//         type: "radio",
//         label: "REM Behaviour disorder symptoms (Acting out dreams)",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "bruxism",
//         type: "radio",
//         label: "Bruxism (grinding teeth in sleep)",
//         options: ["Yes", "No"],
//         required: false,
//       },
//     ],
//   },
//   {
//     page: 4,
//     title: "Co-morbidities and Medications",
//     questions: [
//       {
//         id: "hypertension",
//         type: "radio",
//         label: "Hypertension",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "diabetes",
//         type: "radio",
//         label: "Diabetes",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "ihd",
//         type: "radio",
//         label: "Ischemic Heart Disease",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "stroke",
//         type: "radio",
//         label: "Stroke",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "hypothyroidism",
//         type: "radio",
//         label: "Hypothyroidism",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "neurological_disorder",
//         type: "checkbox",
//         label: "Neurological disorder",
//         options: ["Parkinson’s", "Dementia", "Neuromuscular disease"],
//         required: false,
//       },
//       {
//         id: "respiratory_disorder",
//         type: "checkbox",
//         label: "Chronic Respiratory disorder",
//         options: [
//           "COPD",
//           "Asthma",
//           "Interstitial Lung Disease",
//           "Bronchiectasis",
//         ],
//         required: false,
//       },
//       {
//         id: "surgery_sleep_apnea",
//         type: "radio",
//         label: "Surgery for Sleep Apnea",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "medications",
//         type: "checkbox",
//         label: "Are you taking medications affecting sleep?",
//         options: [
//           "Sedative-hypnotics",
//           "Antidepressants",
//           "Antipsychotics",
//           "Stimulants",
//           "Opioids",
//           "Others",
//         ],
//         required: false,
       
//       },
//       {
//         id: "medications_other",
//         type: "text",
//         label: "Other (please specify)",
//         dependsOn: { id: "medications", value: "Others" },
//         required: false,
//       },
//     ],
//   },
//   {
//     page: 5,
//     title: "Clinical Examination",
//     questions: [
//       { id: "height", type: "number", label: "Height (cm)", required: false },
//       { id: "weight", type: "number", label: "Weight (kg)", required: false },
//       {
//         id: "bmi",
//         type: "number",
//         label: "BMI (kg/sq.m)",
//         calculated: true,
//         dependsOn: ["height", "weight"],
//         formula: "weight / ((height/100) * (height/100))",
//         required: false,
//       },
//       {
//         id: "waist",
//         type: "number",
//         label: "Waist circumference (cm)",
//         required: false,
//       },
//       {
//         id: "hip",
//         type: "number",
//         label: "Hip circumference (cm)",
//         required: false,
//       },
//       {
//         id: "waist_hip_ratio",
//         type: "number",
//         label: "Waist/Hip ratio",
//         calculated: true,
//         dependsOn: ["waist", "hip"],
//         formula: "waist / hip",
//         required: false,
//       },
//       {
//         id: "neck",
//         type: "number",
//         label: "Neck circumference (cm)",
//         required: false,
//       },
//       { id: "bp", type: "text", label: "Blood Pressure", required: false },
//       { id: "spo2", type: "number", label: "SpO2", required: false },
//       {
//         id: "mallampati",
//         type: "radio",
//         label: "Mallampati Grade",
//         options: ["1", "2", "3", "4"],
//         required: false,
//       },
//     ],
//   },
//   {
//     page: 6,
//     title: "Sleepiness Scales",
//     questions: [
//       {
//         id: "epworth_score",
//         type: "number",
//         label: "Epworth Sleepiness Scale Score",
//         required: false,
//       },
//       {
//         id: "iss_q1",
//         type: "radio",
//         label: "On waking, do you feel drowsy/unrefreshing sleep?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q2a",
//         type: "radio",
//         label: "Do you snore?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q2b",
//         type: "radio",
//         label:
//           "If yes, has your bed partner noticed that you stop breathing while snoring?",
//         options: ["Yes", "No"],
//         dependsOn: { id: "iss_q2a", value: "Yes" },
//         required: false,
//       },
//       {
//         id: "iss_q3",
//         type: "radio",
//         label: "Do your knees give way during emotional situations?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q4",
//         type: "radio",
//         label: "Do you kick your bed partner/bed sheets away in sleep?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q5",
//         type: "radio",
//         label: "Do you feel drowsy while watching TV or leisure activities?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q6",
//         type: "radio",
//         label: "Do you feel drowsy / sleepy  while doing household work?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q7",
//         type: "radio",
//         label: "Have you ever experienced sleep attacks?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q8a",
//         type: "radio",
//         label: "Nodded off / Felt sleepy while conversing?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q8b",
//         type: "radio",
//         label: "Nodded off / Felt sleepy  while driving?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q8c",
//         type: "radio",
//         label: "Nodded off / Felt sleepy while cooking?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q8d",
//         type: "radio",
//         label: "Nodded off / Felt sleepy  while office work?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//       {
//         id: "iss_q8e",
//         type: "radio",
//         label: "Nodded off / Felt sleepy  while operating heavy machinery?",
//         options: ["Yes", "No"],
//         required: false,
//       },
//     ],
//   },
//   {
//     page: 7,
//     title: "Clinical Impression & Work-up",
//     questions: [
//       {
//         id: "clinical_impression",
//         type: "checkbox",
//         label: "Clinical Impression",
//         options: [
//           "Poor Sleep Hygiene",
//           "Obstructive Sleep Apnea",
//           "Obesity Hypoventilation Syndrome",
//           "Insomnia",
//           "Restless Legs Syndrome",
//           "Parasomnia",
//           "RBD",
//           "Narcolepsy",
//           "Circadian Rhythm Sleep Disorder",
//           "Shift Work Sleep Disorder",
//           "Others",
//         ],
//         required: false,
//       },
//       {
//         id: "clinical_impression_other",
//         type: "text",
//         label: "Other (please specify)",
//         dependsOn: { id: "clinical_impression", value: "Others" },
//         required: false,
//       },
//       {
//         id: "recommended_workup",
//         type: "checkbox",
//         label: "Recommended work-up",
//         options: [
//           "Level 1 Polysomnography",
//           "Level 2 Polysomnography",
//           "Level 3 Polysomnography",
//           "Split night polysomnography",
//           "Multiple Sleep Latency Test",
//           "PAP titration study",
//         ],
//         required: false,
//       },
//     ],
//   },
// ];
